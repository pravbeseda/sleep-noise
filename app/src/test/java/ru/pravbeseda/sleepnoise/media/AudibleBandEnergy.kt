...
/** Above the random walk's ~3 Hz corner and below every cutoff on trial, so a source lands on one side or the other. */
const val AUDIBLE_BAND_SPLIT_HZ = 200.0

/**
 * Scales the signal to a peak of exactly 1 — the level the mixer's headroom actually constrains — then splits it
 * with a one-pole low-pass at [AUDIBLE_BAND_SPLIT_HZ] and its complementary high-pass and reports the mean energy of the high
 ...
 */
fun highBandEnergyAtUnitPeak(signal: FloatArray): Double {
    val scale = 1.0 / signal.maxOf { abs(it) }
    val smoothing = exp(-2.0 * PI * AUDIBLE_BAND_SPLIT_HZ / SAMPLE_RATE_HZ)
    var low = 0.0
    var highEnergy = 0.0
