 ...
 *
 * Each split is **two** one-poles in series, not one. A single pole falls at 6 dB per octave, which leaves a
...
fun bandEnergyShares(signal: FloatArray, lowSplitHz: Double, highSplitHz: Double): BandShares {
    val lowPass = TwoPoleLowPass(lowSplitHz)
    val midPass = TwoPoleLowPass(highSplitHz)
    var lowEnergy = 0.0
    var midEnergy = 0.0
    var highEnergy = 0.0
    for (sample in signal) {
        val low = lowPass.next(sample.toDouble())
        val aboveLow = sample - low
        val mid = midPass.next(aboveLow)
        lowEnergy += low * low
        midEnergy += mid * mid
...
private class TwoPoleLowPass(cutoffHz: Double) {
    private val smoothing = smoothingFor(cutoffHz)
    private var first = 0.0
    private var second = 0.0

    fun next(sample: Double): Double {
        first = (smoothing * first) + ((1.0 - smoothing) * sample)
        second = (smoothing * second) + ((1.0 - smoothing) * first)
        return second
    }
}
...
private fun smoothingFor(cutoffHz: Double) = exp(-2.0 * PI * cutoffHz / SAMPLE_RATE_HZ)
