...
/**
 * One noise as its controls need to know it: where its level and its checkbox are stored, what the
 * checkbox announces, and how the level reads above the slider.
 */
class NoiseControl(
    val volumeKey: String,
    val enabledKey: String,
    val defaultVolume: Float,
    /** The noise's name, which is what the checkbox announces to a screen reader. */
...
/**
 * One noise's settings: a checkbox that turns the noise off without touching its level, and a slider
 ...
 * noise lab alike — so a new noise gets its checkbox, its spacing and its persistence by existing
...
class NoiseControlView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : LinearLayout(context, attrs) {
     ...
     * lands after [bind] has attached the listeners, so the substituted level and checkbox state would be
     * written into each noise's own preferences — over the level the checkbox exists to preserve.
     *
     * A row therefore saves itself and nothing below it. It loses no state by that: a level and a
     * checkbox state live in the preferences [bind] reads, which is the one place they are kept.
     */
     ...
     *
     * The checkbox never writes over the level, so switching a noise back on brings back what it was.
    ...
    fun bind(noise: NoiseControl, preferences: SharedPreferences, onVolumeChanged: (Float) -> Unit) {
        ...
        noiseToggle.isChecked = preferences.getBoolean(noise.enabledKey, DEFAULT_NOISE_ENABLED)
